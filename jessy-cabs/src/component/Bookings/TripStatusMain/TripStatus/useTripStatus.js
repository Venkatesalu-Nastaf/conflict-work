...
const useDispatched = () => {
    ...
    // const [columnfield,setColumnField]=useState(false)
    ...
    console.log(filteredColumns,"collllll")
  ...
  const handleExcelDownload = async () => {
    ...
    const handlePdfDownload = () => {
      ...
      console.log(header,"new")
        ...
        console.log(header.length,"lenn1",fontdata)
    ...
    //     const header = columns.map(key => ( key.field));
    //     // const header = columns.map(key => ( key.field));
    
    //     // const headerBooking = columns.map(column => column.headerName);
    //     // const headerTripsheet = columns.map(column => column.headerName);
    
    //     // Extracting body for booking
    //     // const bodyBooking = bookingResults.map(row => headerBooking.map(column => row[column.field]));
    
    //     // // Extracting body for tripsheet
    //     // const bodyTripsheet = tripsheetResults.map(row => headerTripsheet.map(column => row[column.field]));
    //     let fontdata = 1;
    //         if (header.length <= 13) {
    //           fontdata = 16;
    //         }
    ...
    //         const body1= bookingResults.map(row => {
    //         const rowData = [];
    //         header.forEach(columnName => {
    //             // Assuming row[columnName] exists and contains the value
    //             rowData.push(row[columnName]);
    ...
    //     const body= tripsheetResults.map(row => {
    //         const rowData = [];
    //         header.forEach(columnName => {
    //             // Assuming row[columnName] exists and contains the value
    //             rowData.push(row[columnName]);
    ...
    //     // Add tripsheet section
    //     pdf.addPage(); // Add a new page for tripsheet
    //     pdf.text("Tripsheet Details", 10, 10); // Heading for tripsheet section
    //     pdf.autoTable({
    //       head: [header],
    //       body: body,
    //       startY: 20,
    ...
    //     // Save the PDF
    //     const pdfBlob = pdf.output('blob');
    //     saveAs(pdfBlob, 'Pending_Reports.pdf');
    // };
    ...
    //         if ( data.tripsheet.length>0 ||data.booking.length>0) {
    ...
    //             // console.log(rowsWithUniqueId,"datarowsid")
    //             setSuccess(true);
      ...
      setColumnShowall(false)
        try {
            ...
            if (data && data.tripsheet && data.booking) {
                ...
                // console.log(combinedRows,"combine")
