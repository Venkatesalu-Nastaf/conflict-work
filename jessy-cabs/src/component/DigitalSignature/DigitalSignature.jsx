    ...
    const DigitalSignature = () => {
        ...
        const [expired, setExpired] = useState(() => {
            const expiredInSessionStorage = sessionStorage.getItem('expired') && localStorage.getItem('expired');
            return expiredInSessionStorage ? JSON.parse(expiredInSessionStorage) : false;
        });
        ...
        const saveSignature = async () => {
            ...
            try {
                await fetch('http://localhost:8081/api/saveSignaturewtid', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
                    body: JSON.stringify({ signatureData: dataUrl, tripId: tripId, uniqueno: uniqueno }),
                });
                clearSignature();
                setTimeout(() => {
                    setExpired(true);
