...
import './CustomerPdfInvoice.css';
import { useData } from "../../../Dashboard/MainDash/Sildebar/DataContext2";
import dayjs from 'dayjs';
import jsPDF from 'jspdf';
import html2canvas from 'html2canvas';
import autoTable from 'jspdf-autotable';
import { useInvoiceCustomer } from './customerInvoiceContext';

const CustomerPdfInvoice = () => {

    const targetRef = useRef()

    const { logo } = useData()

    const { setOpenInvoice } = useInvoiceCustomer()
        ...
        <div>
            <div style={{ display: 'flex', flexDirection: 'column', width: '784px', padding: 20 }} ref={targetRef}>
                ...
                <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', gap: '50px' }}>
                    <div>
                        <h2 className="customerinvoicetext" style={{ textTransform: 'uppercase' }}>jessy cabs</h2>
                        <h2 className="customerinvoicetextno">No.66,ggggggggg</h2>
                    </div>
                    <div className="imagediv">
                        <img src={logo} className="image-customer" alt="organisationimage" />
                    </div>
                </div>

                <div className="mobilediv-customer">
                    <h2 className="organisationtext-customer">Tel : 044-49105959 Mob : 9841505689</h2>
                    <h2 className="organisationtext-ref-customer">GST : 456456AESW</h2>
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', gap: '50px', borderBottom: '1px solid grey', paddingBottom: 5 }}>
                    <div>
                        ...
                        <h2 className="organisationtext-customer">Srinivasan Street Harish,Madipakkam</h2>
                        <h2 className="organisationtext-customer">Andhra Pradesh</h2>
                        <h2 className="organisationtext-customer">GST : 123455678</h2>
                    </div>
                    <div className="Taxinvoicediv-customer">
                        <h3 className="Refnotext-customer">
                            <span>Ref No: </span>
                            <span className="invoice-customer">RF1001</span>
                        </h3>
                    </div>
                </div>

                <div className="Datediv-customer">
                    <p>From: <span className="Datetext-customer">{dayjs(new Date()).format('DD-MM-YYYY')}</span></p>
                    <p>To: <span className="Datetext-customer">{dayjs(new Date()).format('DD-MM-YYYY')}</span></p>
                </div>

            </div>

            <div>
                <table className="table-ref-customer">
                    <thead>
                        <tr>
                            <th className="tableheadtext-customer">SNo</th>
                            <th className="tableheadtext-customer">Bill No</th>
                            <th className="tableheadtext-customer">Bill Date</th>
                            <th className="tableheadtext-customer">Ordered By</th>
                            <th className="tableheadtext-customer">Reported To</th>
                            <th className="tableheadtext-customer">Park/Permit</th>
                            <th className="tableheadtext-customer">Cus Adv</th>
                            <th className="tableheadtext-customer">Total Amount</th>
                        </tr>
                    </thead>

                    <tbody className="tablebody">
                        {invoiceRows.map((row, index) => (
                            <tr className="tabledata-ref-customer" key={index}>
                                <td className="tdata-customer">{index + 1}</td>
                                <td className="tdata-customer">{row.billNo}</td>
                                <td className="tdata-customer">{row.billDate}</td>
                                <td className="tdata-customer">{row.orderedBy}</td>
                                <td className="tdata-customer">{row.reportedTo}</td>
                                <td className="tdata-customer">0</td>
                                <td className="tdata-customer">0</td>
                                <td className="tdata-customer" style={{ textAlign: 'center' }}>0</td>
                            </tr>
                        ))}
                    </tbody>
                    <tfoot>
                        <tr>
                            <td className="tdata-customer"></td>
                            <td className="tdata-customer"></td>
                            <td className="tdata-customer"></td>
                            <td className="tdata-customer"></td>
                            <td className="tdata-customer">Total</td>
                            <td className="tdata-customer">0</td>
                            <td className="tdata-customer">0</td>
                            <td className="tdata-customer" style={{ textAlign: "center" }}>0</td>
                        </tr>
            ...
            <div className="total-div-customer" >
                <div>
            ...
            <div className="printdiv-customer">
                <button onClick={() => setOpenInvoice(false)} className="print-customer">
                    Cancel
                </button>
                <button className="print-customer" onClick={() => handlePrint()} >Print</button>
            </div>
...
export default CustomerPdfInvoice;
