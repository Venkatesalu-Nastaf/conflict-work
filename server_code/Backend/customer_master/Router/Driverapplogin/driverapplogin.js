...
router.post('/drivercreation', (req, res) => {
  ...
  console.log(bookData,"n",userid)

  db.query("select * from drivercreation where userid=?",userid,(err,results)=>{
    if (err) {
    ...
    if(results.length >=1){
        ...
        return res.status(404).json({ message: "userid already exists" });
    }
    else {
